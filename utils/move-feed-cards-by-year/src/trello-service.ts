...
import { BOARD_NAME, LIST_NAME, type DatedCard } from './cards.js';
...
export async function loadSourceFeedBoard(
  api: TrelloApi,
  organization: TrelloOrganization,
): Promise<SourceFeedBoard> {
  const boards = await listOpenBoards(api, organization.id);
  const board = findUniqueNamedResource(boards, BOARD_NAME, 'board', organization.displayName);
  if (!board) {
    throw new Error(`No open "${BOARD_NAME}" board exists in ${organization.displayName}.`);
  }
  ...
  const list = findUniqueNamedResource(
    ...
    LIST_NAME,
    'list',
    `${organization.displayName} / ${BOARD_NAME}`,
  );
  if (!list) {
    throw new Error(
      `Missing required "${LIST_NAME}" list in ${organization.displayName} / ${BOARD_NAME}.`,
    );
  }

  const cards = await listOpenCards(api, list.id);
...
export async function ensureDestination(
  ...
  const current = await inspectDestination(api, organization);
  const board = current.board ?? await createFeedBoard(api, organization.id);

  if (current.list) return { board, list: current.list };
  const list = await createList(api, board.id, LIST_NAME);
  return { board, list };
}
...
export function describeDestinationChanges(plan: DestinationPlan): string {
  if (plan.createBoard) {
    return `Create the "${BOARD_NAME}" board and its "${LIST_NAME}" list.`;
  }
  if (plan.createList) {
    return `Create the "${LIST_NAME}" list on the existing "${BOARD_NAME}" board.`;
  }
  return `Use the existing "${BOARD_NAME}" board and destination list.`;
}
...
async function createFeedBoard(api: TrelloApi, organizationId: string): Promise<TrelloBoard> {
