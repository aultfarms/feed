...
test('loadSourceLivestockBoard resolves both required lists and fetches their cards', async () => {
  const treatmentsCard = card('treatment-card', 'treatments');
  const deadCard = card('dead-card', 'dead');
  const api = new FakeApi(call => {
    ...
    if (call.path === '/boards/board/lists/open') {
      return [ list('treatments', 'Treatments'), list('dead', 'Dead') ];
    }
    if (call.path === '/lists/treatments/cards') return [ treatmentsCard ];
    if (call.path === '/lists/dead/cards') return [ deadCard ];
    throw new Error(`Unexpected request: ${call.method} ${call.path}`);
  });

  const result = await loadSourceLivestockBoard(api, sourceOrganization);

  assert.equal(result.board.id, 'board');
  assert.equal(result.lists.Treatments.id, 'treatments');
  assert.equal(result.lists.Dead.id, 'dead');
  assert.deepEqual(result.cards.Treatments, [ treatmentsCard ]);
  assert.deepEqual(result.cards.Dead, [ deadCard ]);
});

test('inspectDestination rejects ambiguous duplicate Livestock boards', async () => {
  const api = new FakeApi(() => [
    board('first'),
    board('second'),
  ]);
  ...
  await assert.rejects(
    inspectDestination(api, destinationOrganization, 'Treatments'),
    /Found 2 open "Livestock" boards/,
  );
});

test('ensureDestination creates a board without default lists and creates only the selected list', async () => {
  const api = new FakeApi(call => {
    if (call.method === 'get') return [];
    if (call.path === '/boards') return board('created-board');
    if (call.path === '/lists') return list('created-list', 'Dead', 'created-board');
    throw new Error(`Unexpected request: ${call.method} ${call.path}`);
  });

  const result = await ensureDestination(api, destinationOrganization, 'Dead');

  assert.deepEqual(result, {
    board: board('created-board'),
    list: list('created-list', 'Dead', 'created-board'),
  });
...
test('moveCards retries transient errors, paces requests, and reports permanent failures', async () => {
  ...
  const result = await moveCards(
    ...
    [ datedCard('retry'), datedCard('fail'), datedCard('success') ],
    {
      ...
      list: list('destination-list', 'Treatments', 'destination-board'),
    },
    {
