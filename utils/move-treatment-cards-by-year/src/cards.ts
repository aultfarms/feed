import { records } from '@aultfarms/livestock';
import type { TrelloCard } from '@aultfarms/trello';

export const CARD_LIST_NAMES = [ 'Treatments', 'Dead' ] as const;

export type CardListName = typeof CARD_LIST_NAMES[number];
...
export function validateCards(listName: CardListName, cards: TrelloCard[]): CardValidationResult {
  const valid: DatedCard[] = [];
  const errors: CardValidationError[] = [];

  for (const card of cards) {
    const record = listName === 'Treatments'
      ? records.treatmentCardToRecord(card)
      : records.deadCardToRecord(card);
