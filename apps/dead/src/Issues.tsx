...
const IssuePanel = observer(function IssuePanel({ issues }: { issues: ParseIssue[] }) {
  ...
  const validation = card && recordKind && state.records
    ? validateCardNameRepair({
        kind: recordKind,
        card,
        newName: value,
  ...
  const save = async () => {
    ...
    if (configKind) {
      await actions.repairConfigIssue(issue, value);
    } else {
      await actions.repairIssue(issue, value);
    }
  };
      ...
      <div className="issueactions">
        {metadata && (
          <button
            ...
            type="button"
            onClick={() => void actions.openIssueInTrello(issue)}
          >
            Open in Trello
          </button>
        )}
        {metadata && (recordKind || configKind) && (
          <button
            ...
            type="button"
            disabled={!validation?.valid || state.repairing}
            onClick={() => void save()}
          >
            {state.repairing ? 'Saving…' : 'Validate and save'}
          </button>
        )}
      </div>
    </section>
...
export const Issues = observer(function Issues() {
  const { state, actions } = React.useContext(context);
  const allIssues = state.records?.issues || [];
  const warnings = allIssues.filter(issue => issue.severity === 'warning');
  const errorGroups = groupIssues(allIssues.filter(issue => issue.severity === 'error'));
      ...
      <div className="issuesheader">
        ...
        {state.lastRepair && (
          <button
            ...
            onClick={() => void actions.undoLastRepair()}
          >
            Undo last edit
          </button>
        )}
      ...
      {errorGroups.length === 0 ? (
        ...
        <>
          ...
          <div className="issuelist">
            {errorGroups.map((issues, index) => (
              <IssuePanel
                key={`${issues[0]?.card?.id || 'missing'}-${index}`}
                issues={issues}
              />
            ))}
          </div>
